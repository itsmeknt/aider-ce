    ...
    for pattern in glob_patterns:
        for fname in dirname.glob(pattern):
            try:
                ...
                lang_to_results.setdefault(lang, []).append(result)
...
def summarize_results(dirname, stats_languages=None, verbose=True):
    ...
    try:
        tries = max(len(result.get("tests_outcomes", [])) for results in lang_to_results.values() for result in results if result)
    except ValueError:
        tries = 0
    ...
    def add(attr_name, result, global_stats, lang_stats):
        global_prev = getattr(global_stats, attr_name)
        setattr(global_stats, attr_name, global_prev + result)
        
        lang_prev = getattr(lang_stats, attr_name)
        setattr(lang_stats, attr_name, lang_prev + result)
    ...
    for lang, results in lang_to_results.items():
        ...
        for result in results:
            ...
            add("cost", result.get("cost", 0), res, lang_stats)
            add("duration", result.get("duration", 0), res, lang_stats)
            add("test_timeouts", result.get("test_timeouts", 0), res, lang_stats)

            add("error_outputs", result.get("num_error_outputs", 0), res, lang_stats)
            add("user_asks", result.get("num_user_asks", 0), res, lang_stats)
            add("exhausted_context_windows", result.get("num_exhausted_context_windows", 0), res, lang_stats)
            add("num_malformed_responses", result.get("num_malformed_responses", 0), res, lang_stats)
            if result.get("num_malformed_responses"):
                add("num_with_malformed_responses", 1, res, lang_stats)
            add("lazy_comments", result.get("lazy_comments", 0), res, lang_stats)

            add("syntax_errors", result.get("syntax_errors", 0), res, lang_stats)
            add("indentation_errors", result.get("indentation_errors", 0), res, lang_stats)

            add("prompt_tokens", result.get("prompt_tokens", 0), res, lang_stats)
            add("completion_tokens", result.get("completion_tokens", 0), res, lang_stats)

            res.reasoning_effort = result.get("reasoning_effort")
            res.thinking_tokens = result.get("thinking_tokens")
    ...
    if len(lang_to_stats) > 0:
        ...
        for lang in langs:
            ...
            lang_col_width = max(len(lang), len(max([getattr(lang_stats, attr) for attr in lang_stats.__dict__], key=len)))
